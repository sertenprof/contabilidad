// ═══════════════════════════════════════════════
// SISTEMA DE CONTABILIDAD — SUPABASE v2.0
// ═══════════════════════════════════════════════
...
supabase.auth.onAuthStateChange(function (event, session) {
  if (event.session && !user_id) handleAuth();
  if (!session && user_id) location.reload();
});

// ══════════════════════════
// ADAPTADOR DE BASE DE DATOS SUPABASE
// ══════════════════════════
...
async function handleStockLogic(datos) {
  ...
  // 1. Buscar si ya existe ese producto en Inventario Final
  ...
  if (existing) {
    ...
    var newCant = Number(existing.cantidad || 0) + cant;
    await supabase
      .from('inventario_final')
      .update({ 'cantidad': newCant, 'precio': precio, 'valor_total': newCant * precio })
      .eq('id', existing.id);
    toast('Stock actualizado en Inventario Final', 's');
  } else {
    // Si no existe, crearlo
    await supabase.from('inventario_final').insert([{ producto: producto, um: um, cantidad: cant, precio: precio, valor_total: cant * precio, mes: mes, user_id: user_id }]);
    toast('Producto agregado al Inventario Final', 's');
  }
}

// ══════════════════════════
// CÁLCULOS EN EL NAVEGADOR (Ultra rápido)
// ══════════════════════════
function calcEstadoLocal(mes, vR, cR, gR, iiR, ifR) {
  var m = mes || null;
  var ventas = 0;
  vR.forEach(function (r) { if (!m || String(r['mes'] || '').trim() === String(m).trim()) ventas += Number(r['transferencia'] || 0) + Number(r['efectivo'] || 0) + Number(r['otros_activos'] || 0); });
  ...
  var ii = 0, iff = 0;
  iiR.forEach(function (r) { if (!m || String(r['mes'] || '').trim() === String(m).trim()) ii += Number(r['valor_total'] || 0); });
  ifR.forEach(function (r) { if (!m || String(r['mes'] || '').trim() === String(m).trim()) iff += Number(r['valor_total'] || 0); });
...
async function calcDashboardLocal(mes) {
  var res = await Promise.all([
    sv('getSheetData', 'ventas'), sv('getSheetData', 'costos'), sv('getSheetData', 'gastos'),
    sv('getSheetData', 'inventario_inicial'), sv('getSheetData', 'inventario_final'), sv('readConfig')
  ]);
...
function go(s) {
  ...
  for (var i = 0; i < links.length; i++) links[i].classList.toggle('on', links[i].dataset.s === s);
  var secs = document.querySelectorAll('.sec');
  for (var j = 0; j < secs.length; j++) secs[j].classList.toggle('on', secs[j].id === 's-' + s);
...
function kpi(l, v, s, c, cls) {
  return '<div class="kpi ' + c + '"><div class="kpi-l">' + l + '</div><div class="kpi-v">' + v + '</div>' + (s ? '<div class="kpi-s ' + (cls || '') + '">' + s + '</div>' : '') + '</div>';
}
...
function mb(label, pct, c) {
  ...
  return '<div class="mb-i"><label>' + label + ' <span>' + fp(pct) + '</span></label><div class="mb-track"><div class="mb-fill ' + c + '" style="width:' + cl + '%"></div></div></div>';
}
...
function drawBar(e) {
  ...
  for (var j = 0; j < vals.length; j++) {
    var x = pad.l + gap + (barW + gap) * j, bH = (vals[j] / maxV) * cH, yy = pad.t + cH - bH;
    ctx.fillStyle = colors[j]; ctx.beginPath(); var rad = Math.min(4, barW / 4);
    ctx.moveTo(x, yy + rad); ctx.arcTo(x, yy, x + rad, yy, rad); ctx.arcTo(x + barW, yy, x + barW, yy + rad, rad); ctx.lineTo(x + barW, pad.t + cH); ctx.lineTo(x, pad.t + cH); ctx.closePath(); ctx.fill();
    ctx.fillStyle = '#666'; ctx.font = '10px Outfit,sans-serif'; ctx.textAlign = 'center'; ctx.fillText(labels[j], x + barW / 2, H - 10);
    ctx.fillStyle = '#333'; ctx.font = '600 10px Outfit,sans-serif'; ctx.fillText(f(vals[j]), x + barW / 2, yy - 4);
  }
}
...
function drawDon(e) {
  ...
  for (var i = 0; i < vals.length; i++) {
    ...
    ctx.beginPath(); ctx.arc(cx, cy, R, angle, angle + slice); ctx.arc(cx, cy, r, angle + slice, angle, true); ctx.closePath(); ctx.fillStyle = colors[i]; ctx.fill();
    var mid = angle + slice / 2, lx = cx + Math.cos(mid) * (R + 14), ly = cy + Math.sin(mid) * (R + 14);
    ctx.fillStyle = '#444'; ctx.font = '10px Outfit,sans-serif';
    ctx.textAlign = mid > Math.PI / 2 && mid < Math.PI * 1.5 ? 'right' : 'left';
    ctx.fillText(labels[i] + ' ' + fp(vals[i] / total * 100), lx, ly + 3); angle += slice;
  }
  ctx.fillStyle = '#ffffff'; ctx.beginPath(); ctx.arc(cx, cy, r - 1, 0, Math.PI * 2); ctx.fill();
  ctx.fillStyle = '#1b1b1b'; ctx.font = '800 18px Outfit,sans-serif'; ctx.textAlign = 'center'; ctx.fillText(f(total), cx, cy + 1);
  ctx.fillStyle = '#999'; ctx.font = '10px Outfit,sans-serif'; ctx.fillText('Total Gastos', cx, cy + 16);
}

// ══════════════════════════
// TABLAS CRUD
// ════════════════════════════════════
function loadTbl(table, tb, fn) {
  sv('getSheetData', table).then(function (data) {
    var rows = curMes ? data.r.filter(function (r) { return String(r['mes'] || '').trim() === String(curMes).trim(); }) : data.r;
    var el = $(tb);
    if (!rows.length) { el.innerHTML = '<tr><td colspan="20" class="empty"><i class="fas fa-inbox"></i>Sin registros para este periodo</td></tr>'; return; }
...
function ab(sh, r) {
  return '<td class="acts">' +
    '<button class="bt bt-s bt-i" onclick="openEdit(\'' + sh + '\',' + r.id + ')" title="Editar"><i class="fas fa-pen"></i></button>' +
    '<button class="bt bt-d bt-i" onclick="delRow(\'' + sh + '\',' + r.id + ')" title="Eliminar"><i class="fas fa-trash"></i></button></td>';
}

function rV(r) {
  var t = Number(r['transferencia'] || 0) + Number(r['efectivo'] || 0) + Number(r['otros_activos'] || 0);
  return '<tr><td>' + fd(r['fecha']) + '</td><td>' + (r['descripcion'] || '') + '</td><td>' + (r['tipo'] || '') + '</td><td class="n">' + f(r['transferencia']) + '</td><td class="n">' + f(r['efectivo']) + '</td><td class="n">' + f(r['otros_activos']) + '</td><td class="n" style="font-weight:700">' + f(t) + '</td><td>' + mn(r['mes']) + '</td>' + ab('ventas', r) + '</tr>';
}

function rG(r) {
  return '<tr><td>' + fd(r['fecha']) + '</td><td>' + (r['descripcion'] || '') + '</td><td>' + (r['subcategoria'] || '') + '</td><td class="n">' + f(r['monto']) + '</td><td>' + (r['tipo'] || '') + '</td><td>' + (r['categoria'] || '') + '</td><td>' + (r['factura'] || '-') + '</td>' + ab('gastos', r) + '</tr>';
}

function rC(r) {
  return '<tr><td>' + fd(r['fecha']) + '</td><td>' + (r['descripcion'] || '') + '</td><td class="n">' + f(r['monto']) + '</td><td>' + (r['clasificacion'] || '') + '</td><td>' + (r['tipo'] || '') + '</td><td>' + (r['factura'] || '-') + '</td>' + ab('costos', r) + '</tr>';
}

function rCo(r) {
  var stock = String(r['stock'] || '').trim();
  var sh = stock === 'Sí' ? '<span style="color:var(--accent);font-weight:700"><i class="fas fa-check-circle"></i> Sí</span>' : '<span style="color:var(--muted)">No</span>';
  return '<tr><td>' + fd(r['fecha']) + '</td><td>' + (r['producto'] || '') + '</td><td>' + (r['cantidad'] || '') + '</td><td>' + (r['um'] || '') + '</td><td class="n">' + f(r['precio']) + '</td><td class="n" style="font-weight:700">' + f(r['monto']) + '</td><td>' + sh + '</td>' + ab('compras', r) + '</tr>';
}
...
function loadInv() {
  Promise.all([sv('getSheetData', 'inventario_inicial'), sv('getSheetData', 'inventario_final')]).then(function (results) {
    ...
    var ir = function (r, shN) {
      return '<tr><td>' + (r['producto'] || '') + '</td><td>' + (r['um'] || '') + '</td><td class="n">' + f(r['precio']) + '</td><td class="n" style="font-weight:700">' + f(r['valor_total']) + '</td>' + ab(shN, r) + '</tr>';
    };
    $('tbII').innerHTML = ii.r.filter(flt).map(function (r) { return ir(r, 'inventario_inicial'); }).join('') || '<tr><td colspan="6" class="empty"><i class="fas fa-inbox"></i>Sin datos</td></tr>';
    $('tbIF').innerHTML = iff.r.filter(flt).map(function (r) { return ir(r, 'inventario_final'); }).join('') || '<tr><td colspan="6" class="empty"><i class="fas fa-inbox"></i>Sin datos</td></tr>';
  }).catch(function (err) { toast('Error: ' + err.message, 'e'); });
}
...
function loadEst() {
  sv('loadDashboard', curMes || null).then(function (d) {
    ...
    var row = function (l, v, c) { return '<tr class="' + (c || '') + '"><td>' + l + '</td><td class="n">' + v + '</td></tr>'; };
    var note = function (t) { return '<tr><td colspan="2" class="note">' + t + '</td></tr>'; };
    var sl = function (t) { return '<tr class="sl"><td colspan="2">' + t + '</td></tr>'; };
    var ib = 'Por cada peso vendido te quedan ' + Math.round(e.mb) + ' centavos disponibles para cubrir otros gastos.';
    var io = 'La rentabilidad de la operacion antes de impuestos y gastos financieros es ' + (e.mo > 30 ? 'alta.' : 'baja.');
    var in_ = 'Por cada peso vendido conservas ' + Math.round(e.mn) + ' centavos de Ganancia.';
    if (e.mn >= 15) in_ += ' Empresa altamente competitiva y con gran capacidad de reinversion.';
    $('tbE').innerHTML =
      row('Ventas Netas', f(e.ventas)) +
      row('Costos de Venta', f(e.cv), 'ind') + row('Utilidad Bruta', f(e.ub), 'sub') + row('Margen Bruto', fp(e.mb), 'mr') + note(ib) +
      sl('Gastos') + row('Gastos Operativos', f(e.go), 'ind') + row('Gastos Financieros', f(e.gf), 'ind') + row('Gastos Generales y de Administracion', f(e.ga), 'ind') +
      row('Total Gastos', f(e.gt), 'sub') + row('Utilidad Operativa', f(e.uo), 'sub') + row('Margen Operativo', fp(e.mo), 'mr') + note(io) +
      row('Impuestos', f(e.imp), 'ind') + row('Utilidad Neta', f(e.un), 'big') + row('Margen Neto', fp(e.mn), 'mr') + note(in_) +
      sl('Desglose de Costos') + row('Costos Fijos', f(e.cf), 'ind') + row('Costos Variables', f(e.cvar), 'ind') + row('Total Costos', f(e.cv), 'sub') +
      sl('Inventario') + row('Inventario Inicial', f(e.ii), 'ind') + row('Inventario Final', f(e.iff), 'ind');
  }).catch(function (err) { toast('Error: ' + err.message, 'e'); });
}
...
function loadRat() {
  sv('loadDashboard', curMes || null).then(function (d) {
    var r = d.ratios; var ml = curMes ? MESES[Number(curMes)] : 'Todos los meses';
    function hc(t, items) {
      return '<div class="rt-c"><div class="rt-c-t">' + t + ' — ' + ml + '</div>' +
        items.map(function (i) { return '<div class="rt-r"><span class="l">' + i[0] + '</span><span class="v ' + (i[2] || '') + '">' + i[1] + '</span></div>'; }).join('') + '</div>';
    }
    function rc(v, g, b) { return v >= g ? 'ok' : (v <= b ? 'no' : 'wa'); }
    $('rtG').innerHTML =
      hc('Ratios de Liquidez', [['Liquidez Corriente', fr(r.lc), rc(r.lc, 1.5, 0.8)], ['Tesoreria', fr(r.tes), rc(r.tes, 0.5, 0.1)], ['Prueba Acida', fr(r.pa), rc(r.pa, 1, 0.5)], ['Liquidez Inmediata', fr(r.li), rc(r.li, 0.2, 0.05)]]) +
      hc('Ratios de Rentabilidad', [['ROE (Retorno sobre Patrimonio)', fp(r.roe), rc(r.roe, 15, 0)], ['ROA (Retorno sobre Activos)', fp(r.roa), rc(r.roa, 5, 0)]]) +
      hc('Ratios de Solvencia', [['Endeudamiento', fp(r.end), r.end < 50 ? 'ok' : (r.end > 80 ? 'no' : 'wa')], ['Cobertura de Intereses', fr(r.ci), rc(r.ci, 3, 1)]]) +
      hc('Ratios de Eficiencia', [['Rotacion de Inventarios', fr(r.ri), ''], ['Rotacion de CPC', fr(r.rcpc), '']]) +
      hc('Otros Indicadores', [['Efectivo', f(r.ef)], ['Equivalentes de Efectivo', f(r.eq)], ['Cheques en transito', f(0)], ['Cuentas bancarias de ahorro', f(0)], ['Activos Corrientes', f(r.ac)], ['Pasivos Corrientes', f(r.pc)], ['Patrimonio Neto', f(r.pn)], ['Activos Totales', f(r.at)], ['Pasivo Total', f(r.pt)], ['Inventario Promedio', f(r.ip)], ['Ventas a Credito', f(r.vc)], ['Cuentas PC Promedio', f(r.cpp)]]));
}).catch (function (err) { toast('Error: ' + err.message, 'e'); });
}
...
function loadCfg() {
  sv('readConfig').then(function (d) {
    $('cfgG').innerHTML = d.r.map(function (r) {
      return '<div class="cfg-i"><label>' + (r['parametro'] || '') + '</label><input type="number" step="0.01" data-k="' + (r['parametro'] || '') + '" value="' + (r['valor'] || 0) + '"></div>';
    }).join('');
  }).catch(function (err) { toast('Error: ' + err.message, 'e'); });
}
...
function opMo(title, html, onSv) {
  $('moT').textContent = title; $('moB').innerHTML = html;
  $('moSv').className = 'bt bt-p'; $('moSv').innerHTML = '<i class="fas fa-check"></i> Guardar';
  $('moSv').onclick = onSv;
...
function moSel() {
  return '<option value="">-- Mes --</option>' + MESES.slice(1).map(function (m, i) { return '<option value="' + (i + 1) + '">' + m + '</option>'; }).join('');
}
function selOpt(opts, sel) {
  return opts.map(function (o) { return '<option' + (String(sel || '').trim() === String(o).trim() ? ' selected' : '') + '>' + o + '</option>'; }).join('');
}
...
function getFormData() {
  ...
  var d = {};
  for (var i = 0; i < inputs.length; i++) d[inputs[i].name] = inputs[i].value;
  return d;
}
...
function openEdit(sh, id) {
  sv('getSheetData', sh).then(function (data) {
    var r = null;
    for (var i = 0; i < data.r.length; i++) { if (data.r[i].id === id) { r = data.r[i]; break; } }
    if (!r) return;
    editCtx = { sh: sh, id: id };
...
function saveFromForm(sec) {
  ...
  if (editCtx && SH_MAP[sec]) {
    sv('editRow', editCtx.sh, editCtx.id, d).then(function () { toast('Registro actualizado', 's'); clMo(); load(curSec); }).catch(function (err) { toast('Error: ' + err.message, 'e'); });
  } else {
...
function saveInv(sh) {
  ...
  if (editCtx) {
    sv('editRow', editCtx.sh, editCtx.id, d).then(function () { toast('Registro actualizado', 's'); clMo(); load('inventario'); }).catch(function (err) { toast('Error: ' + err.message, 'e'); });
  } else {
    sv('addRow', sh, d).then(function () { toast('Registro agregado', 's'); clMo(); load('inventario'); }).catch(function (err) { toast('Error: ' + err.message, 'e'); });
  }
}

function delRow(sh, id) {
  opMo('Confirmar eliminacion',
    ...
    function () {
      sv('removeRow', sh, id).then(function () { toast('Registro eliminado', 's'); clMo(); load(curSec); }).catch(function (err) { toast('Error: ' + err.message, 'e'); });
    });
